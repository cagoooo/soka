...
### **v1.1.1 - Perceptual Experience & Console Hygiene (感知體驗與介面淨化)** (Current)
- 🎨 **Enhanced Loading Screen**：使用 `framer-motion` 打造具備 Aurora 背景與玻璃質感的載入螢幕，優化感知性能。
- 🧹 **Console Hygiene**：徹底消除主控台警告，包含圖表尺寸計算異常與無障礙表單屬性。
- 🔒 **Firestore Multi-tab Sync**：啟用多分頁同步管理機制，解決快取競爭問題。

### **v1.1.0 - Engineering Excellence (工程體質優化)**
- 🏗️ **Core Stability**：
